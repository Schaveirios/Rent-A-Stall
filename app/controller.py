...
def AddTenants2():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            stallno1 = form.stallno.data
            stallType = form.stalltype.data
            date_started = from2.datestarted.data
            type = Types.query.filter_by(stall_type=stallType).first()
            ...
            if stallnum:
                if stallnum.stall_status == "1":
                    ...
                    if stall:                        
                        ...
                        rents = Rents(date_started = datestarted)
                        if form.tenantphotoID.data:
                            stall.stall_status = "1"
                            dbase.session.add(stall)
                            dbase.session.commit()
...
def deadline():
    tID = Pays.query.filter_by(tenantID=id).first()
    if tID.month == 'January':
         
    elif tID.month == 'February':

    elif tID.month == 'March':

    elif tID.month == 'April':
    ...
    elif tID.month == 'August':

    elif tID.month == 'September':

    elif tID.month == 'October':

    elif tID.month == 'November':

    elif tID.month == 'December':
